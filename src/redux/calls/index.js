...
export const deleteCalls = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      ...
      return {
        data: { id },
        message: response.data.message || "Call deleted successfully",
      };
    } catch (error) {
      return thunkAPI.rejectWithValue(
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCalls.rejected, (state, action) => {
        ...
        toast.error(action.payload.message || "Failed to fetch calls");
      })
      ...
      .addCase(addCalls.fulfilled, (state, action) => {
        ...
        state.calls = {
          ...state.calls,
          data: [action.payload.data, ...state.calls.data],
        };
        state.success = action.payload.message;
        toast.success(action.payload.message || "Call added successfully");
      })
      .addCase(addCalls.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload.message;
        toast.error(action.payload.message || "Failed to add call");
      })
      ...
      .addCase(updateCalls.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          state.calls = {
            ...state.calls,
            data: [...state.calls, action.payload.data],
          };
        }
        state.success = action.payload.message;
        toast.success(action.payload.message || "Call updated successfully");
      })
      .addCase(updateCalls.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload.message;
        toast.error(action.payload.message || "Failed to update call");
      })
      ...
      .addCase(deleteCalls.fulfilled, (state, action) => {
        ...
        state.success = action.payload.message;
        toast.success(action.payload.message || "Call deleted successfully");
      })
      .addCase(deleteCalls.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload.message;
        toast.error(action.payload.message || "Failed to delete call");
      })
      ...
      .addCase(fetchRelatedTo.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload.message;
        toast.error(action.payload.message || "Failed to fetch related data");
      });
  },
});
