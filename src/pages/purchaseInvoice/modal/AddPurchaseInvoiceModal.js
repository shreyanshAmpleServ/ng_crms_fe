...
import {
  addSalesInvoice,
  fetchSalesInvoiceCode,
  updateSalesInvoice,
} from "../../../redux/salesInvoice";
import {
  addPurchaseInvoice,
  fetchPurchaseInvoiceCode,
  updatePurchaseInvoice,
} from "../../../redux/purchaseInvoice";
...
const AddInvoiceModal = ({ order, setOrder }) => {
  ...
  React.useEffect(() => {
    if (order) {
      ...
      setItemNumber(
        order?.purchase_invoice_items?.map((item) => ({
  ...
  const onSubmit = async (data) => {
    ...
    try {
      order
        ? await dispatch(updatePurchaseInvoice(formData))
  ...
  React.useEffect(() => {
    const offcanvasElement = document.getElementById(
      "offcanvas_add_edit_purchase_invoice"
    );
    if (offcanvasElement) {
