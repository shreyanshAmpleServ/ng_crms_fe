...
import { addSalesInvoice, fetchSalesInvoiceCode, updateSalesInvoice } from "../../../redux/salesInvoice";
import { addPurchaseInvoice, fetchPurchaseInvoiceCode, updatePurchaseInvoice } from "../../../redux/purchaseInvoice";
...
const AddInvoiceModal = ({order,setOrder}) => {
  ...
  const {orderCode ,loading } = useSelector((state)=>state.purchaseInvoices)
...
setItemNumber(order?.purchase_invoice_items?.map((item)=>({
  ...
  useEffect(() => {
    ...
    dispatch(fetchPurchaseInvoiceCode())
...
const handleAvatarChange = (e) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      order ? await dispatch(updatePurchaseInvoice( formData))
      :  await dispatch(addPurchaseInvoice(formData)).unwrap();
  ...
  React.useEffect(() => {
        const offcanvasElement = document.getElementById("offcanvas_add_edit_purchase_invoice");
        if (offcanvasElement) {
  ...
  >
    <div className="offcanvas-header border-bottom">
      <h4>{order ? "Update " :"Add New "} Purchase Invoice</h4>
      <button
        type="button"
              ...
              <div className="d-flex align-items-center justify-content-between">
               ...
               />
                {errors.cust_id && (
                  ...
                  {/* Order Code  */}
            <div className=" col-md-6 mb-3">
                <label className="col-form-label">Invoice Code</label>
                <input
                  type="text"
                  disabled
