...
setItemNumber(order?.purchase_invoice_items?.map((item)=>({
  ...
  React.useEffect(() => {
        const offcanvasElement = document.getElementById("offcanvas_preview_purchase_invoice");
        if (offcanvasElement) {
  ...
  return (
    ...
    tabIndex={-1}
    id="offcanvas_preview_purchase_invoice"
  >
...
>
  {order?.status === "L" ? "Closed" : order?.status === "C" ? "Canceled" : order?.status === "P" ? "Pending" : "Open"}
          ...
          <div className="fw-bold  h1">Purchase Invoice</div>
          ...
          <div className="row">
            ...
            <div className="">
                ...
                <div className="text-wrap"> {order?.billto  || ""}</div>
            ...
            <div className="">
                ...
                <div lassName="text-wrap"> {order?.shipto || ""}</div>
                <div lassName="text-wrap"> {order?.address || ""}</div>
            </div>
            </div>
            ...
            <div className="">
                ...
                <div lassName="text-wrap"> {order?.purchase_invoice_vendor?.name  || ""}</div>
                <div lassName="text-wrap">{` ${order?.purchase_invoice_vendor?.billing_street  || ""} ${order?.purchase_invoice_vendor?.billing_city || ""}  ${order?.purchase_invoice_vendor?.state?.name  || ""} ${order?.purchase_invoice_vendor?.country?.name || ""}, ${order?.purchase_invoice_vendor?.billing_zipcode || ""}`}</div>
            </div>
            </div>
            {/* <div className="col-md-4 ">
            <div className="">
