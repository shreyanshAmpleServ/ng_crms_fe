...
setItemNumber(order?.order_items?.map((item)=>({
  ...
  >
    <div   className=" border-bottom offcanvas-header justify-content-between">
      ...
      <h4>{order ? "Preview" :"Add New "} Order</h4>
...
>
  {order?.status === "L" ? "Closed" : order?.status === "C" ? "Canceled" : order?.status === "P" ? "Pending" : "Open"}
          ...
          <div className="fw-bold h1"> Order  <span className="text-primary h3">#{order?.order_code}</span></div>
          ...
          <div className="row">
            ...
            <div className="text-break">
                ...
                <div lassName="text-wrap"> {order?.billto || ""}</div>
            ...
            <div className="text-break">
                ...
                <div lassName=" text-break"> {order?.shipto || ""}</div>
                <div lassName="text-wrap"> {order?.address || ""}</div>
            </div>
            </div>
            ...
            <div className="text-break">
                ...
                <div lassName="text-break text-wrap"> {order?.order_vendor?.name || ""}</div>
                <div lassName="text-wrap">{` ${order?.order_vendor?.billing_street || ""},${order?.order_vendor?.billing_city || ""}, ${order?.order_vendor?.state?.name || ""}, ${order?.order_vendor?.country?.name || ""}, ${order?.order_vendor?.billing_zipcode || ""}`}</div>
            </div>
            </div>
            {/* <div className="col-md-4 ">
            <div className="">
