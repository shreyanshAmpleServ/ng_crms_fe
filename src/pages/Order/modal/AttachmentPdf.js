...
import { fetchOrderById } from "../../../redux/order";
// import { fetchOrderById } from "../../../redux/quotation";
...
const PreviewPdf = ({ id, setAttachments,setPrevPdf }) => {
  ...
  React.useEffect(() => {
   ...
   const { orderDetail , loading, success } = useSelector(
      (state) => state.orders,
    );
  ...
  React.useEffect(() => {
    if (orderDetail) {
      ...
      setOptionalItem(orderDetail?.optional_items ? JSON.parse(orderDetail?.optional_items) : "");
      
      // Pre-fill email subject
      setEmailForm(prev => ({
        ...prev,
        subject: `order ${orderDetail?.order_code} - ${orderDetail?.order_vendor?.name}`,
        to: orderDetail?.order_vendor?.email || '',
        body:`<p style="font-family: Arial, sans-serif; font-size: 14px;">Dear <strong>${orderDetail?.order_vendor?.name || 'Customer'}</strong>,</p>
        <br />

    <p style="font-family: Arial, sans-serif; font-size: 14px;">       Please find attached order <strong>${orderDetail?.order_code}</strong> for your review.</p>
        <br />
           <p>You can also <a href="${process.env.REACT_APP_API_BASE_URL}/crms/order-pdf/${btoa(orderDetail?.id?.toString())}" target="_blank" style="color:#007bff;text-decoration:underline;">click here to view it online</a> and reply to confirm your order.</p> <br />
  ...
  const { toPDF, targetRef } = usePDF({
    filename: `${orderDetail?.order_code}.pdf`,
  ...
  useEffect(() => {
    const generatePdfAttachment = async () => {
        try {
          ...
          const attachment = {
            filename: `${orderDetail?.order_code || "order"}.pdf`,
            ...
            <div ref={targetRef} id="pdf-preview-container"  className="card-body p-5 position-relative overflow-none" style={{overflow:"hidden"}}>
              ...
              <div className="d-flex justify-content-between align-items-start mb-3">
                <div className="p-5 py-1">
                  <h1 className="display-6 fw-bold text-primary mb-0">order</h1>
                  <p className="text-muted fs-5">#{orderDetail?.order_code}</p>
                </div>
                <div className="text-end">
                  <div className="card bg-light border-0 p-3" style={{ minWidth: '250px' }}>
                    <div className="mb-2">
                      <span className="fw-bold text-dark">Issue Date:</span>
                      <span className="ms-2">{moment(orderDetail?.createdate).format("DD-MM-YYYY")}</span>
                    </div>
                    <div>
                      <span className="fw-bold text-dark">Due Date:</span>
                      <span className="ms-2">{moment(orderDetail?.due_date).format("DD-MM-YYYY")}</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="card h-100 border-0 bg-light shadow">
                    <div className="card-body">
                      ...
                      <div className="text-break">
                        <div className="fw-semibold">{orderDetail?.billto || "Not specified"}</div>
                        <div className="text-muted">{orderDetail?.address || ""}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="col-md-4">
                  <div className="card h-100 border-0 bg-light shadow">
                    <div className="card-body">
                      ...
                      <div className="text-break">
                        <div className="fw-semibold">{orderDetail?.shipto || "Same as Bill To"}</div>
                        <div className="text-muted">{orderDetail?.address || ""}</div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="col-md-4">
                  <div className="card h-100 border-0 bg-dark-gradient shadow">
                    <div className="card-body">
                      ...
                      <div className="text-break">
                        <div className="fw-semibold">{orderDetail?.order_vendor?.name || ""}</div>
                        <div className="opacity-75">
                          {`${orderDetail?.order_vendor?.billing_street || ""}${orderDetail?.order_vendor?.billing_city ? ', ' + orderDetail?.order_vendor?.billing_city : ""}${orderDetail?.order_vendor?.state?.name ? ', ' + orderDetail?.order_vendor?.state?.name : ""}${orderDetail?.order_vendor?.country?.name ? ', ' + orderDetail?.order_vendor?.country?.name : ""}${orderDetail?.order_vendor?.billing_zipcode ? ', ' + orderDetail?.order_vendor?.billing_zipcode : ""}`}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="card border-primary shadow-lg">
                    ...
                    <div className="card-body">
                      <div className="d-flex justify-content-between mb-2">
                        ...
                        <span className="fw-semibold">{formatNumber(orderDetail?.total_bef_tax)}</span>
                      </div>
                      <div className="d-flex justify-content-between mb-2">
                        <span>Total Discount:</span>
                        <span className="fw-semibold">{formatNumber(orderDetail?.disc_prcnt)}</span>
                      </div>
                      <div className="d-flex justify-content-between mb-2">
                        <span>
                          Rounded
                          <input
                            type="checkbox"
                            className="ms-2"
                            checked={orderDetail?.rounding === "Y"}
                            readOnly
                          />
                        </span>
                        <span className="fw-semibold">
                          {orderDetail?.rounding === "Y"
                            ? formatNumber(orderDetail?.rounding_amount)
                            : formatNumber(orderDetail?.rounding_amount) || 0}
                        </span>
                      </div>
                      <div className="d-flex justify-content-between mb-2">
                        <span>Total Tax Amount:</span>
                        <span className="fw-semibold">{formatNumber(orderDetail?.tax_total) || 0}</span>
                      </div>
                      <hr />
                      <div className="d-flex justify-content-between">
                        ...
                        <span className="h5 fw-bold text-primary">
                          {orderDetail?.rounding === "Y"
                            ? formatNumber(orderDetail?.rounding_amount)
                            : formatNumber(orderDetail?.total_amount)}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              {orderDetail?.terms && (
                <div className="mb-4">
                  <div className="card">
                    ...
                    <div className="card-body">
                      <div dangerouslySetInnerHTML={{ __html: JSON.parse(orderDetail?.terms) }} />
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Other Items */}
              {orderDetail?.other_items && JSON.parse(orderDetail?.other_items)?.map((value, index) => (
              ...
              <div className="card-header bg-gradient text-white border-0"
                ...
                <div className="d-flex justify-content-between align-items-center">
                  <div className="d-flex align-items-center">
                    ...
                    <div>
                      <h4 className="mb-1 fw-bold">Send order Email</h4>
                      <p className="mb-0 opacity-75">Send order PDF to customer</p>
                    </div>
                  </div>
                  <button 
                ...
                <form onSubmit={handleEmailSubmit}>
                  ...
                  <div className="mb-4">
                    <div className="alert alert-info d-flex align-items-center">
                      ...
                      <span>
                        <strong>Attachment:</strong> {orderDetail?.order_code}.pdf will be automatically attached
                      </span>
                    </div>
                  </div>
