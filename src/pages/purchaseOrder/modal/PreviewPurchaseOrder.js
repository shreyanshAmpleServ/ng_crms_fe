...
setItemNumber(order?.purchase_order_items?.map((item)=>({
  ...
  React.useEffect(() => {
        const offcanvasElement = document.getElementById("offcanvas_preview_purchase_order");
        if (offcanvasElement) {
  ...
  return (
    ...
    tabIndex={-1}
    id="offcanvas_preview_purchase_order"
  >
...
>
  {order?.status === "L" ? "Closed" : order?.status === "C" ? "Canceled" : order?.status === "P" ? "Pending" : "Open"}
          ...
          <div className="fw-bold  h1">Purchase Order</div>
          ...
          <div className="row">
            ...
            <div className="text-break">
                ...
                <div> {order?.purchase_order_vendor?.name  || ""}</div>
                <div>{` ${order?.purchase_order_vendor?.billing_street  || ""},${order?.purchase_order_vendor?.billing_city || ""}, ${order?.purchase_order_vendor?.state?.name  || ""}, ${order?.purchase_order_vendor?.country?.name || ""}, ${order?.purchase_order_vendor?.billing_zipcode || ""}`}</div>
            </div>
            </div>
            {/* <div className="col-md-4 ">
            <div className="">
